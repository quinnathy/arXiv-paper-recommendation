...
class TestInitV2:
    ...
    def test_single_seed(self):
        ...
        centroids, labels, tw = init_user_profile_v2([s])
    ...
    def test_two_close_seeds_merge(self):
        ...
        centroids, labels, tw = init_user_profile_v2([s1, s2])
        assert centroids.shape[0] == 1, "Close seeds should merge into 1 thread"
        assert np.isclose(tw.sum(), 1.0)
    ...
    def test_two_distant_seeds_separate(self):
        ...
        centroids, labels, tw = init_user_profile_v2([s1, s2])
        assert centroids.shape[0] == 2, "Distant seeds should remain separate"
        assert len(labels) == 2
        assert np.isclose(tw.sum(), 1.0)
    ...
    def test_max_threads_cap(self):
        ...
        centroids, labels, tw = init_user_profile_v2(seeds, max_threads=3)
        assert centroids.shape[0] <= 3
        assert len(labels) == centroids.shape[0]
        assert np.isclose(tw.sum(), 1.0)
    ...
    def test_support_seed_does_not_form_thread(self):
        ...
        centroids, labels, tw = init_user_profile_v2([core, support])
        assert centroids.shape[0] == 1, "Support seed should merge into core thread"

    def test_all_centroids_unit_norm(self):
        rng = np.random.default_rng(7)
        seeds = [_make_seed(label=f"s{i}", rng=rng, split_power=0.8) for i in range(4)]
        centroids, _, _ = init_user_profile_v2(seeds)
    ...
    def test_thread_weights_sum_to_one(self):
        ...
        _, _, tw = init_user_profile_v2(seeds)
        assert np.isclose(tw.sum(), 1.0)
    ...
    def test_fallback_all_support(self):
        ...
        centroids, labels, tw = init_user_profile_v2(seeds)
    ...
    def test_thread_label_from_highest_specificity(self):
        ...
        _, labels, _ = init_user_profile_v2([s_low, s_high])
        assert labels[0] == "specific"
...
class TestDBThreadMetadata:
    ...
    def test_create_without_thread_fields(self, tmp_path):
        ...
        assert user is not None
        assert user["thread_labels"] is None
        assert user["thread_weights"] is None
